import type { SVGProps } from "react";

export function IconLayoutVertical(props: SVGProps<SVGSVGElement>) {
  return (
    <svg
      data-name="icon-layout-vertical"
      viewBox="0 0 79.86 51.14"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <title>IconLayoutVertical</title>
      <path
        d="M39.22 15.99h-8.16c-.79 0-1.43-.67-1.43-1.5s.64-1.5 1.43-1.5h8.16c.79 0 1.43.67 1.43 1.5s-.64 1.5-1.43 1.5z"
        opacity={0.75}
      />
      <rect
        height={2.73}
        opacity={0.5}
        rx={1.36}
        ry={1.36}
        width={16.72}
        x={29.63}
        y={18.39}
      />
      <path
        d="M75.1 6.68v1.45c0 .63-.49 1.14-1.09 1.14H30.72c-.6 0-1.09-.51-1.09-1.14V6.68c0-.62.49-1.14 1.09-1.14h43.29c.6 0 1.09.52 1.09 1.14z"
        opacity={0.9}
      />
      <rect
        height={19.95}
        opacity={0.4}
        rx={2.11}
        ry={2.11}
        width={21.8}
        x={29.63}
        y={24.22}
      />
      <g strokeLinecap="round" strokeMiterlimit={10}>
        <rect
          height={3.42}
          opacity={0.32}
          rx={0.33}
          ry={0.33}
          width={2.01}
          x={61.06}
          y={38.15}
        />
        <rect
          height={6.58}
          opacity={0.44}
          rx={0.33}
          ry={0.33}
          width={2.01}
          x={56.78}
          y={34.99}
        />
        <rect
          height={8.7}
          opacity={0.53}
          rx={0.33}
          ry={0.33}
          width={2.01}
          x={65.17}
          y={32.86}
        />
        <rect
          height={12.4}
          opacity={0.66}
          rx={0.33}
          ry={0.33}
          width={2.01}
          x={69.55}
          y={29.17}
        />
      </g>
      <g opacity={0.5}>
        <circle cx={63.17} cy={18.63} r={7.5} />
        <path d="M63.17 11.63c3.86 0 7 3.14 7 7s-3.14 7-7 7-7-3.14-7-7 3.14-7 7-7m0-1c-4.42 0-8 3.58-8 8s3.58 8 8 8 8-3.58 8-8-3.58-8-8-8z" />
      </g>
      <g opacity={0.74}>
        <path d="M64.05 18.13l3.38-5.67c.93.64 1.7 1.48 2.26 2.47.56.98.89 2.08.96 3.21h-6.6z" />
        <path d="M67.57 13.19a6.977 6.977 0 012.52 4.44h-5.17l2.65-4.44m-.31-1.43l-4.1 6.87h8c0-1.39-.36-2.75-1.04-3.95a8.007 8.007 0 00-2.86-2.92z" />
      </g>
      <g strokeLinecap="round" strokeMiterlimit={10}>
        <rect
          height={40}
          opacity={0.8}
          rx={2}
          ry={2}
          width={19.14}
          x={5.84}
          y={5.02}
        />
        <g stroke="#fff">
          <path
            d="M9.02 17.39L21.25 17.39"
            fill="none"
            opacity={0.72}
            strokeWidth="2px"
          />
          <path
            d="M9.02 24.6L19.54 24.6"
            fill="none"
            opacity={0.48}
            strokeWidth="2px"
          />
          <path
            d="M9.02 20.88L18.4 20.88"
            fill="none"
            opacity={0.55}
            strokeWidth="2px"
          />
          <circle cx={10.98} cy={9.91} fill="#fff" opacity={0.8} r={2.54} />
          <path
            d="M15.53 8.65L21.25 8.65"
            fill="none"
            opacity={0.8}
            strokeWidth="2px"
          />
          <path d="M15.32 11.3L20.38 11.3" fill="none" opacity={0.6} />
        </g>
      </g>
    </svg>
  );
}

// export function IconLayoutCompact(props: SVGProps<SVGSVGElement>) {
//   return (
//     <svg
//       data-name="icon-layout-compact"
//       viewBox="0 0 79.86 51.14"
//       xmlns="http://www.w3.org/2000/svg"
//       {...props}
//     >
//       <title>IconLayoutCompact</title>
//       <rect
//         height={40}
//         rx={2}
//         ry={2}
//         strokeLinecap="round"
//         strokeMiterlimit={10}
//         width={4}
//         x={5.84}
//         y={5.2}
//       />
//       <g stroke="#fff" strokeLinecap="round" strokeMiterlimit={10}>
//         <path
//           d="M7.26 11.56L8.37 11.56"
//           fill="none"
//           opacity={0.66}
//           strokeWidth="2px"
//         />
//         <path
//           d="M7.26 14.49L8.37 14.49"
//           fill="none"
//           opacity={0.51}
//           strokeWidth="2px"
//         />
//         <path
//           d="M7.26 17.39L8.37 17.39"
//           fill="none"
//           opacity={0.52}
//           strokeWidth="2px"
//         />
//         <circle cx={7.81} cy={7.25} fill="#fff" opacity={0.8} r={1.16} />
//       </g>
//       <path
//         d="M15.81 14.49L22.89 14.49"
//         fill="none"
//         opacity={0.75}
//         strokeLinecap="round"
//         strokeMiterlimit={10}
//         strokeWidth="3px"
//       />
//       <rect
//         height={2.73}
//         opacity={0.5}
//         rx={0.64}
//         ry={0.64}
//         strokeLinecap="round"
//         strokeMiterlimit={10}
//         width={22.19}
//         x={14.93}
//         y={18.39}
//       />
//       <rect
//         height={2.73}
//         opacity={0.9}
//         rx={0.64}
//         ry={0.64}
//         strokeLinecap="round"
//         strokeMiterlimit={10}
//         width={59.16}
//         x={14.93}
//         y={5.89}
//       />
//       <rect
//         height={19.95}
//         opacity={0.4}
//         rx={2.11}
//         ry={2.11}
//         strokeLinecap="round"
//         strokeMiterlimit={10}
//         width={32.68}
//         x={14.93}
//         y={24.22}
//       />
//       <g strokeLinecap="round" strokeMiterlimit={10}>
//         <rect
//           height={3.42}
//           opacity={0.32}
//           rx={0.33}
//           ry={0.33}
//           width={2.01}
//           x={59.05}
//           y={38.15}
//         />
//         <rect
//           height={6.58}
//           opacity={0.44}
//           rx={0.33}
//           ry={0.33}
//           width={2.01}
//           x={54.78}
//           y={34.99}
//         />
//         <rect
//           height={8.7}
//           opacity={0.53}
//           rx={0.33}
//           ry={0.33}
//           width={2.01}
//           x={63.17}
//           y={32.86}
//         />
//         <rect
//           height={12.4}
//           opacity={0.66}
//           rx={0.33}
//           ry={0.33}
//           width={2.01}
//           x={67.54}
//           y={29.17}
//         />
//       </g>
//       <g opacity={0.5}>
//         <circle cx={62.16} cy={18.63} r={7.5} />
//         <path d="M62.16 11.63c3.86 0 7 3.14 7 7s-3.14 7-7 7-7-3.14-7-7 3.14-7 7-7m0-1c-4.42 0-8 3.58-8 8s3.58 8 8 8 8-3.58 8-8-3.58-8-8-8z" />
//       </g>
//       <g opacity={0.74}>
//         <path d="M63.04 18.13l3.38-5.67c.93.64 1.7 1.48 2.26 2.47.56.98.89 2.08.96 3.21h-6.6z" />
//         <path d="M66.57 13.19a6.977 6.977 0 012.52 4.44h-5.17l2.65-4.44m-.31-1.43l-4.1 6.87h8c0-1.39-.36-2.75-1.04-3.95a8.007 8.007 0 00-2.86-2.92z" />
//       </g>
//     </svg>
//   );
// }

export function IconLayoutHorizontal(props: SVGProps<SVGSVGElement>) {
  return (
    <svg
      data-name="icon-layout-horizontal"
      viewBox="0 0 79.86 51.14"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <title>IconLayoutHorizontal</title>
      <path
        d="M6.85 14.49L15.02 14.49"
        fill="none"
        opacity={0.75}
        strokeLinecap="round"
        strokeMiterlimit={10}
        strokeWidth="3px"
      />
      <rect
        height={2.73}
        opacity={0.5}
        rx={0.64}
        ry={0.64}
        strokeLinecap="round"
        strokeMiterlimit={10}
        width={25.6}
        x={5.84}
        y={18.39}
      />
      <rect
        height={2.73}
        opacity={0.9}
        rx={0.64}
        ry={0.64}
        strokeLinecap="round"
        strokeMiterlimit={10}
        width={68.26}
        x={5.84}
        y={5.89}
      />
      <rect
        height={19.95}
        opacity={0.4}
        rx={2.11}
        ry={2.11}
        strokeLinecap="round"
        strokeMiterlimit={10}
        width={37.71}
        x={5.84}
        y={24.22}
      />
      <g strokeLinecap="round" strokeMiterlimit={10}>
        <rect
          height={3.42}
          opacity={0.32}
          rx={0.33}
          ry={0.33}
          width={2.01}
          x={59.05}
          y={38.15}
        />
        <rect
          height={6.58}
          opacity={0.44}
          rx={0.33}
          ry={0.33}
          width={2.01}
          x={54.78}
          y={34.99}
        />
        <rect
          height={8.7}
          opacity={0.53}
          rx={0.33}
          ry={0.33}
          width={2.01}
          x={63.17}
          y={32.86}
        />
        <rect
          height={12.4}
          opacity={0.66}
          rx={0.33}
          ry={0.33}
          width={2.01}
          x={67.54}
          y={29.17}
        />
      </g>
      <g opacity={0.5}>
        <circle cx={62.16} cy={18.63} r={7.5} />
        <path d="M62.16 11.63c3.86 0 7 3.14 7 7s-3.14 7-7 7-7-3.14-7-7 3.14-7 7-7m0-1c-4.42 0-8 3.58-8 8s3.58 8 8 8 8-3.58 8-8-3.58-8-8-8z" />
      </g>
      <g opacity={0.74}>
        <path d="M63.04 18.13l3.38-5.67c.93.64 1.7 1.48 2.26 2.47.56.98.89 2.08.96 3.21h-6.6z" />
        <path d="M66.57 13.19a6.977 6.977 0 012.52 4.44h-5.17l2.65-4.44m-.31-1.43l-4.1 6.87h8c0-1.39-.36-2.75-1.04-3.95a8.007 8.007 0 00-2.86-2.92z" />
      </g>
    </svg>
  );
}
