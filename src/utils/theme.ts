...
export const getThemeTokenVariants = (propertyPath: string) => {
  ...
  const val = keys.reduce<Record<string, any> | undefined>((obj, key) => {
...
export const removePx = (value: string | number): number => {
  ...
  // 检查是否以 px 结尾（不区分大小写）
  const hasPx = PX_REGEX.test(trimmed);
